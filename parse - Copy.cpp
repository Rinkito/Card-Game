...
int main() {
  ...
  //3 sorting stack nodes initialize
	...
	{
		push(tail_hand,head_hand,counter_hand,/*pop [node]->suit?, pop [node]->number?*/)
	}
  ...
  cout << "or 4 to place first card in hand back into bottom of deck." << endl;
	...
	while(bool game_end == false)
	{
		...
		{
			push_deck(tail_deck, first_deck, counter_deck, size_deck,tail_hand->suit, tail_hand->number); // suit and number comes from hand on all of these
